...
def generate(shape, seed):
    ...
    # the total is always exactly NNZ. (Column indices themselves are not
    # needed by this problem; only the row structure matters.)
    base = NNZ // R
    rem = NNZ - base * R
    ...
    scale = rng.standard_normal(R).astype(np.float32)

    return [values, row_ptr.astype(np.int32), scale]
