...
Implement the pairwise repulsive Coulomb-force accumulation for `N` charged
bodies laid out on a line (1D positions), stored as a flat `float32` array
`pos` of length `N`.
...
`1.5` power, to avoid a singularity when two bodies coincide). Note the sign
convention: `d` is `pos[i] - pos[j]` (not `pos[j] - pos[i]`), which makes the
force repulsive (body `i` is pushed away from every other body `j`).
...
- `outputs[0]` is the flattened output array, of the same length `N`;
  `outputs[0][i]` must hold the accumulated repulsive force for body `i`.
